...
export async function summarizeAnswer(query: Query, rows: any[]): Promise<string> {
  ...
  const metricColumn = METRIC_COL_MAP[query.metric];
  
  const includeFullStatLine = query.task === 'compare';
  ...
  const formattedRows = topRows
    .map((row, index) => {
      ...
      const metricValue = row[query.metric] || row[metricColumn];
  ...
  const metricName = query.metric.replace('_', ' ').toUpperCase();
